...
for centroid_id, word_id, dist in tqdm.tqdm(zip(centroid_ids, vals, dists)):
    freq_mat[centroid_id, word_id] += np.exp(-dist + min_dist)

freq_mat = csr_matrix(freq_mat)

sparse.save_npz(ckpt_path + 'cluster_count_softmax.npz', freq_mat)

freq_mat = sparse.load_npz(ckpt_path + 'cluster_count_softmax.npz')
...
sparse.save_npz(ckpt_path + 'cluster_freq_softmax.npz', freq_mat)
...
df.to_csv(ckpt_path + 'cluster_softmax.csv')
