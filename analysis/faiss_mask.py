...
for temp in np.arange(2.0, 15.1, 0.1):
    ...
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask.npy' + str(temp), np.concatenate(all_probs))
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_knns.npy' + str(temp), np.concatenate(all_knns, axis=0))
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_dists.npy' + str(temp), np.concatenate(all_dists))
