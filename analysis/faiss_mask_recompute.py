...
for temp in np.arange(0.1, 6.1, 0.1):
    all_probs = []
    all_knns = []
    all_dists = []
    temp = float(temp)
    ...
    for batch_idx in tqdm.tqdm(range(num_batches)):
        ...
        qsize = batch_queries.shape
        knns_vecs = torch.from_numpy(keys[knns]).cuda().view(qsize[0], topk, -1)
        query_vecs = batch_queries.cuda().view(qsize[0], 1, qsize[1]).repeat(1, topk, 1)
    ...
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_recomp.npy' + str(temp), np.concatenate(all_probs))
    # np.save(dstore_filename.split('/')[-1] + '_faiss_mask_recomp_knns.npy' + str(temp), np.concatenate(all_knns, axis=0))
    # np.save(dstore_filename.split('/')[-1] + '_faiss_mask_recomp_dists.npy' + str(temp), np.concatenate(all_dists))
