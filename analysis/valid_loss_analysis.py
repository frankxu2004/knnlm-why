...
tokens = np.load('tokens.npy').astype(np.int32)
tgt_len = tokens.size
skipped_toks = 0
...
def process_scores(score_filename):
    ...
    print('valid loss:', total_loss/count, 'ppl:', np.exp(total_loss/count))

process_scores('scores.npy')
process_scores('knnlm_scores.npy')
process_scores('best_knn_only_scores.npy')
process_scores('overfit_valid_scores.npy')
process_scores('overfit129_valid_scores.npy')
