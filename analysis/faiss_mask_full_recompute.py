...
for batch_idx in tqdm.tqdm(range(num_batches)):
    ...
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_full_recomp_knns.npy', np.concatenate(all_knns, axis=0))
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_full_recomp_dists.npy', np.concatenate(all_dists, axis=0))
