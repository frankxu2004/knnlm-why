...
score_files_dict = {
    'real_mask': glob.glob('dstore_subsampled_0.05_real_mask.npy*.npy'),
    'faiss_flat_mask': glob.glob('dstore_subsampled_0.05_faiss_mask_flat.npy*.npy'),
    'faiss_mask': glob.glob('dstore_subsampled_0.05_faiss_mask.npy*.npy'),
    'faiss_mask_nprobe': glob.glob('dstore_subsampled_0.05_faiss_mask_nprobe.npy*.npy'),
    'faiss_mask_recomp': glob.glob('dstore_subsampled_0.05_faiss_mask_recomp.npy*.npy'),
}

for name in score_files_dict:
    extra_score_files = score_files_dict[name]
    with open(f'{name}_softmax_temperature.txt', 'w') as outfile:
