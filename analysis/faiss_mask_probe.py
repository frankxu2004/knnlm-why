...
for temp in np.arange(0.1, 15.1, 0.1):
    all_probs = []
    all_knns = []
    all_dists = []
    temp = float(temp)
    ...
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_nprobe.npy' + str(temp), np.concatenate(all_probs))
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_nprobe_knns.npy' + str(temp), np.concatenate(all_knns, axis=0))
    np.save(dstore_filename.split('/')[-1] + '_faiss_mask_nprobe_dists.npy' + str(temp), np.concatenate(all_dists))
